...
const DeleteExpenseModal = ({ open, onClose, onConfirm, expense }) => {
  if (!open) return null;
    ...
    <div className="modal-add-expense-overlay" onClick={onClose}>
      <div className="modal-delete-expense-content" onClick={e => e.stopPropagation()} tabIndex={-1} aria-modal="true" role="dialog">
        ...
        <div style={{display:'flex',gap:12,justifyContent:'flex-end'}}>
          <button onClick={onClose} style={{padding:'8px 18px',borderRadius:6,border:'none',background:'#f4f5fb',color:'#4e54c8',fontWeight:600,fontSize:'1.06em',cursor:'pointer'}}>Cancel</button>
          <button onClick={onConfirm} style={{padding:'8px 18px',borderRadius:6,border:'none',background:'linear-gradient(90deg,#e53e3e,#fbb6b6)',color:'#fff',fontWeight:700,fontSize:'1.06em',cursor:'pointer'}}>Delete</button>
        </div>
      </div>
    </div>,
    document.body
